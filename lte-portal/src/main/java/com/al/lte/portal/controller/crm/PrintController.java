...
//				response.sendRedirect(request.getContextPath() + "/error/500.jsp");
	 ...
	 * @return
	 */
	@RequestMapping(value = "/invoiceprintbymonth", method = RequestMethod.GET)
	@AuthorityValid(isCheck = false)
	public String prepareOwingCharge(){
	    return "/print/invoiceprint_bymonth";
	}
     ...
     */
    @SuppressWarnings("unchecked")
	@RequestMapping(value = "/invoiceNotice", method = RequestMethod.POST)
    @ResponseBody
	public JsonResponse invoiceNotice(@RequestBody Map<String, Object> paramMap,
			@LogOperatorAnn String flowNum, 
			HttpServletRequest request, HttpServletResponse response) {
    	SessionStaff sessionStaff = (SessionStaff) ServletUtils
				.getSessionAttribute(super.getRequest(), SysConstant.SESSION_KEY_LOGIN_STAFF);
		try {
			...
			String Oper_Date = f.format(c.getTime());
			paramMap.put("Oper_Date", Oper_Date);
			Map<String, Object> returnMap = printBmo.invoiceNotice(paramMap, flowNum, sessionStaff);
			if(ResultCode.R_SUCCESS.equals(MapUtils.getString(returnMap, "resultCode", ""))){
				...
				if(ResultCode.R_SUCCESS.equals(MapUtils.getString(resultMap, "code", ""))){
					return super.successed(MapUtils.getString(resultMap, "message", "获取打印成功信息失败！"));	
				}else 
					return super.failed(MapUtils.getString(resultMap, "message", "获取错误信息失败！"),ResultConstant.FAILD.getCode());
				}
			else{				
				return super.failed(
						returnMap.get("resultMsg"),
						ResultConstant.FAILD.getCode());
			}				
		} catch (BusinessException be) {
			this.log.error("门户/print/invoiceNotice方法be异常", be);
			return super.failed(be);
		} catch (InterfaceException ie) {
			return super.failed(ie, paramMap, ErrorCode.INVOICENOTICE);
		} catch (Exception e) {
			log.error("门户/print/invoiceNotice方法异常", e);
			return super.failed(ErrorCode.INVOICENOTICE, e, paramMap);
		}
    }
     ...
     */
    @SuppressWarnings("unchecked")
	@RequestMapping(value = "/invoiceReverse", method = RequestMethod.POST)
    @ResponseBody
	public JsonResponse invoiceReverse(@RequestBody Map<String, Object> paramMap,
			@LogOperatorAnn String flowNum, 
			HttpServletRequest request, HttpServletResponse response) {
    	SessionStaff sessionStaff = (SessionStaff) ServletUtils
				.getSessionAttribute(super.getRequest(), SysConstant.SESSION_KEY_LOGIN_STAFF);
		try {
			...
			String Oper_Date = f.format(c.getTime());
			paramMap.put("Oper_Date", Oper_Date);
			Map<String, Object> returnMap = printBmo.invoiceReverse(paramMap, flowNum, sessionStaff);
			if(ResultCode.R_SUCCESS.equals(MapUtils.getString(returnMap, "resultCode", ""))){
				Map<String, Object> resultMap = (Map<String, Object>) returnMap.get("resultMap");
				if(ResultCode.R_SUCCESS.equals(MapUtils.getString(resultMap, "code", ""))){
					return super.successed(MapUtils.getString(resultMap, "message", "获取打印成功信息失败！"));	
			...
			this.log.error("门户/print/invoiceReverse方法be异常", be);
			return super.failed(be);
		} catch (InterfaceException ie) {
			return super.failed(ie, paramMap, ErrorCode.INVOICEREVERSE);
		} catch (Exception e) {
			log.error("门户/print/invoiceReverse方法异常", e);
			return super.failed(ErrorCode.INVOICEREVERSE, e, paramMap);
		}
    }
}
