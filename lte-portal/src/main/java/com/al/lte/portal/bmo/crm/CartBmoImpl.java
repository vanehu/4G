...
public class CartBmoImpl implements CartBmo{
	 ...
	 * 用户停复机列表查询 
	 */
	public Map<String, Object> queryTfjCarts(Map<String, Object> dataBusMap,String optFlowNum, SessionStaff sessionStaff) throws Exception{
		Map<String, Object> result = new HashMap<String, Object>();
		DataBus db = InterfaceClient.callService(dataBusMap,PortalServiceCode.QUERY_SA_DEAL_RECORDS, optFlowNum, sessionStaff);
		try{
			result.put("code", -1);
			result.put("mess", db.getResultMsg());
			if (ResultCode.R_SUCC.equals(StringUtils.defaultString(db.getResultCode()))) {
			...
			throw new BusinessException(ErrorCode.QUERY_SA_DEAL_RECORDS,dataBusMap,db.getReturnlmap(), e);
		}
	}
}
