 ...
 * @copyRight 亚信联创EC研发部
 */
@Controller("com.al.lte.portal.controller.crm.ProdController")
@RequestMapping("/prod/*")
public class ProdController extends BaseController {
	...
	public String preProdQuery(HttpSession session,Model model){
		...
		return "/product/product-detail-query";
	}
					...
					if(result.get("offerProd2Parties")!=null){
						...
						if(offerProd2Parties.size()>0){
							...
							if(!MapUtils.getMap(offerProd2Party, "party", empty).isEmpty()){
								...
								return "product/product-detail-list";
							}
						}
						else{
							model.addAttribute("flag", 1);
	...
	@RequestMapping(value = "/changePaymentType", method = {RequestMethod.POST})
	public String changePaymentType(@RequestBody Map<String, Object> param, Model model, HttpServletResponse response) {
		model.addAttribute("prodId",param.get("prodId"));
		return "/order/order-change-paymenttype";
	}
}
