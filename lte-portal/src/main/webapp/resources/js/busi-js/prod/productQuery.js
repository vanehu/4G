...
product.query = (function(){
	...
	var _changeCustIdType = function(option){
		...
		if(type==0){
			...
			$("#cust").attr("placeHolder","请输入合法证件号码");
			$("#cust").attr("data-validate","validate(digits:请准确填写证件号码) on(blur)");
		}else{
	...
	var _queryProduct = function(page){
		...
		var param = {
				...
				pageSize : _pageSize,
				areaId : $("#p_areaId").val(),
				ifCancel : $("#p_ifCancel").attr("checked") ? "Y" : "N" //是否查询拆机用户
			};
					
		if($("#accurate").attr("checked")){
			if($("#p_areaId_val").val()==""){
				$.alert("提示","请先选择所属地区再进行查询");
				return;
			}
		...
		$.callServiceAsHtmlGet(contextPath+"/prod/prodQuery", param, {
			...
			"done":function(response){
				...
				$("#prodList").html(response.data).show(function(){
					if(param.ifCancel == "Y"){
						$(".numberList thead tr td:last-child").hide();
