...
bill.charge = (function(){
	...
	_initQryForm = function(){
		$('#cashChargeForm').on('formIsValid',function(event,form){
			if($.trim($("#phoneNumber").val())!=$.trim($("#confirmNumber").html().replace(/[ ]/g,""))){
				$.alert("提示","充值号码与确认号码不符，可能是您的浏览器脚本出现了问题，请刷新页面或返回上一步重试");
				return;
			}
...
/*		if($("#areaName").val()==""){
	...
	var _prepareChargeReceipt = function(a, _areaId, _acctNbr, _reqSerial){
		...
		*/
		...
		receiptInfo.payMethod = "现金";                                                                                           //付费方式
		receiptInfo.sumCharge = $.trim($(a).parent().parent().find("td:eq(0)").text());         //费用总额（单位：元）
		...
		var chargeItem = {
				acceNumber : _acctNbr,
				itemName : $.trim($(a).parent().parent().find("td:eq(1)").text()),
				charge : $.trim($(a).parent().parent().find("td:eq(0)").text())
		};
		_chargeItems.push(chargeItem);
		receiptInfo.chargeItems = _chargeItems;                                                                         //费用项列（部分转售商的模板可能带有子报表，展示费用项列）
...
/*		if($.trim($("#invoiceNbr").val())==""){
	...
	//导入批量信息Excel
	var _batchInfoImport = function(){		
		var fileUpload = $.trim($("#fileUpload").val());
		if(fileUpload!=""){
			var fileSuffix = fileUpload.substr(fileUpload.lastIndexOf(".") + 1);
			if(fileSuffix=="xls" || fileSuffix=="xlsx"){
				$.ecOverlay("<strong>批量信息校验中，请稍后...</strong>");
				//批量充值【输入校验】
				$("#batchChargeInfo").submit();
			}else{
				$.alert("提示","导入的文件类型错误，后缀必须为xls或xlsx！");
				return;
			}
		}else{
	...
	//批量充值【充值鉴权】【批量充值】
	var _batchDoCash = function(){
		$.ecOverlay("<strong>批量信息校验中，请稍后....</strong>");
		var canDoCash = true;//能否开始批量充值的标识，若有一条用户信息未能通过充值鉴权则将该标识置为假
		var chargeList = [];//待充值列表
		var authFailList = [];//鉴权失败列表
		//发送充值鉴权的请求
		$("#chargeAuthList tr").each(function(){
			var _rowNum = $.trim($(this).find("td:eq(0)").text());
			var _phoneNumber = $.trim($(this).find("td:eq(1)").text());
			var _chargeAmount = $.trim($(this).find("td:eq(2)").text());
			var authParams = {
					phoneNumber : _phoneNumber,
					destinationAttr : "2",   //用户属性 2:移动电话
					queryItemType : "0",     //查询余额类型 0:总余额 1:可提取余额
			...
			var authResult = $.callServiceAsJson(contextPath+"/bill/charge/cashConfirm", authParams);
			if(authResult.code==0){//鉴权成功，将该条信息加入待充值列表
				var chargeItem = {
						phoneNumber : _phoneNumber,
						chargeAmount : _chargeAmount
				};
				chargeList.push(chargeItem);
			}else{//鉴权失败，将充值标识置为假并将该条信息加入鉴权失败列表中
				canDoCash = false;
				var authFailItem = {
						rowNum : _rowNum,
						phoneNumber :_phoneNumber,
						chargeAmount : _chargeAmount,
						errorMsg : authResult.data
				};
				authFailList.push(authFailItem);
			}
		});
		
		var chargeParams = {};
		if(canDoCash){
			chargeParams.canDoCash = "Y";
			chargeParams.chargeList = chargeList;
		}else{
			chargeParams.canDoCash = "N";
			chargeParams.authFailList = authFailList;
		}
		$.callServiceAsHtml(contextPath+"/bill/charge/batchDoCash", JSON.stringify(chargeParams), {
			"always" : function(){
				$.unecOverlay();
			},
			"done" : function(response){
				$("#batchDoCashReturnInfo").html(response.data);
			}
		});
	};
	
	//批量充值模板下载
	var _modelDownload = function(){
		location.href=contextPath+"/file/BatchChargeModel.xls";
	};
	
	//重置上传的批量信息
	var _batchInfoReset = function(){
		$("#fileUpload").val("");
		$("#batchDoCashReturnInfo").html("");
	};
	...
	return {
		...
		batchInfoImport : _batchInfoImport,
		batchDoCash : _batchDoCash,
		modelDownload : _modelDownload,
		batchInfoReset : _batchInfoReset
	};
