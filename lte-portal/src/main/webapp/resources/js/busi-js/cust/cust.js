...
order.cust = (function(){
	...
	var _certTypeByPartyType = function(_partyTypeCd,id){
	   ...
	   if (response.code === '' && queryCertTypeRetryCount < 3) {
		   queryCertTypeRetryCount++;
		   _certTypeByPartyType(_partyTypeCd,id);
		   return;
	   } else {
		   queryCertTypeRetryCount = 0;
	   }
	   if(response.code==0){
					...
					if(data!=undefined && data.length>0){
						...
						if(id=="identidiesTypeCd" && idType=="OFF")
						{
//							$("#"+id+" option[value='1']").remove();
	...
	var _custidentidiesTypeCdChoose = function(scope,id) {
		...
		if(identidiesTypeCd==-1){
			...
			$("#"+id).attr("data-validate","validate(digits:请准确填写证件号码) on(blur)");
		}else{
...
//		$('#p_cust_identityNum').attr("disabled",isID&&(!isIdTypeOff));
	...
	$('#custQueryFirstForm').off().bind('formIsValid', function(event, form) {
		...
		if(identityCd!=-1||CONST.getAppDesc()==0){
		 identityCd=$("#p_cust_identityCd").val();
		 authFlag="1";
		}else{
		 authFlag="0";
	...
	var _custReset = function() {
		...
		$("#p_cust_areaId_val").val(OrderInfo.staff.areaAllName);
		$("#p_cust_areaId").val(OrderInfo.staff.areaId);
		//定位客户
		OrderInfo.boCusts.prodId=-1;
		OrderInfo.boCusts.partyId="";
		OrderInfo.boCusts.partyProductRelaRoleCd="0";
...
//			$("#readCertBtnID").hide();
	...
	createCustInfo = {
			...
			segmentId:($.trim($("#partyTypeCd  option:selected").val())==1) ? "1100":"1000"
		};
	...
	OrderInfo.boCustInfos.segmentId=createCustInfo.segmentId;
	...
	OrderInfo.cust = {
		...
		segmentId:createCustInfo.segmentId
	};
	//客户属性
