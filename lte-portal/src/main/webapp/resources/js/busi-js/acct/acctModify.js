...
//		} 	
		...
		//帐户信息细节显示或隐藏初始化
		var jr = $.callServiceAsJson(contextPath+"/acct/ifCanAdjustCreditLimit", {});
		//根据工号权限是否展示信用额度调控
		if(jr.data=="0"){
			$("#credit").show();
		}else{
		...
		boAccountItemsDEL = {
				itemSpecId : CONST.ACCT_ATTR.CREDIT_LIMIT,  //账户属性：信用额度 - 规格ID
				value : $.trim($("#creditLimit").val())*100,
				state : "DEL"
		};
		if($("#postType").val()!=-1){
			boAccountMailingDEL = {
	...
	//新建账户时账户信用额度属性初始化（账户信息修改不用）
	var _creditLimit = function(){
		//先查询工号权限：能否调整账户信用额度
		var jr = $.callServiceAsJson(contextPath+"/acct/ifCanAdjustCreditLimit", {});
		var creditLimit;
		if(jr.data=="0"){
			creditLimit = "<li class='full'><label style='width:90px;'>信用额度 (元)：</label><input type='text' class='inputWidth250px' id='creditLimit' style='width:150px;'/><label class='f_red'>*</label></li>";
		}else{
			creditLimit = "<li class='full' style='display:none;'><label style='width:90px;'>信用额度 (元)：</label><input type='text' class='inputWidth250px' id='creditLimit' style='width:150px;'/><label class='f_red'>*</label></li>";
		}
		$("#defineNewAcct").find("li:eq(0)").after(creditLimit);
		//再查询额度默认值配置
		var response = $.callServiceAsJson(contextPath+"/acct/defaultCreditLimit", {});
		$("#creditLimit").val(response.data);
	};
	...
	var _acctInfoModify_Submit = function() {
		...
		//账户属性节点 - 账户信用额度
		var boAccountItemsADD = {
				itemSpecId : CONST.ACCT_ATTR.CREDIT_LIMIT,  //账户属性：信用额度 - 规格ID
				value : $.trim($("#creditLimit").val())*100,
				state : "ADD"
		};
		_busiOrder.data.boAccountItems.push(boAccountItemsDEL);
		_busiOrder.data.boAccountItems.push(boAccountItemsADD);
		//账单投递信息节点
