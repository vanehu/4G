			...
			selectStr += optionStr + "</select><br>"; 
			 ...
			 if(param.dataTypeCd==1){  //文本框
				if(param.rule==undefined){
					...
					if(param.rule.isConstant=='Y'){ //不可修改
						if(itemSpecId == CONST.ITEM_SPEC.LITTLE_PAY){
							var id=prodId+'_'+specId+'_'+itemSpecId;
							selectStr += '<br>'+param.name + ' : <input id="'+prodId+'_'+specId+'_'+itemSpecId
							+'" class="inputWidth183px" type="text" disabled="disabled" value="'+paramVal+'" >';
							if(ec.util.isObj(specId)&&OrderInfo.isGroupProSpecId(specId)){//是否是群功能产品
								var type=CONST.getGroupServProdMap(specId);
								selectStr+='<a class="purchase" href="javascript:order.main.queryGroup('+type+',0,\''+id+'\');">选择</a>';
							}
//							selectStr+='<div class="easyDialogdiv" style="width:980px;height:460px;" id="div_group_dialog">';
//										+'</div>';
							selectStr+='<br>';
						}else{
					   ...
					   if(param.rule.isOptional=="N") { //必填
    						 if(itemSpecId == '20020048' && _isDynamicStored){ // 促销 - 动态预存款
                                  param.name = param.name + '(元)';
                                  _isDynamicStored = false;
                             }
							selectStr += param.name + ' : <input id="'+prodId+'_'+itemSpecId  
			...
			if(ec.util.isArray(data.result.servSpec)){
				for (var i = 0; i < data.result.servSpec.length; i++) {
					...
					if(servSpec.ifDault==0){
						...
						var newSpec = CacheData.getServSpec(prodId,servSpec.servSpecId); //已开通里面查找
						if(ec.util.isObj(newSpec)){
							...
							AttachOffer.addOpenServList(prodId,servSpec.servSpecId,servSpec.servSpecName);*/
	...
	//根据付费类型过滤附属销售品，主套餐的feeType为空，就不进行过滤，附属销售品 feeTypeCd 为空，就不行展示
	var _filterOfferByFeeType=function(data){
		if(ec.util.isObj(data)){
			if(ec.util.isArray(data.result.offerSpec)){
				var feeType=AttachOffer.obtainFeeType();
				var offerSpecs=[];
				if(ec.util.isObj(feeType)){
					$.each(data.result.offerSpec,function(){
						if(AttachOffer.isEqualsFeeType(feeType,this.feeTypeCd)){
							offerSpecs.push(this);
						}
					});
					data.result.offerSpec=[];//清空
					data.result.offerSpec=offerSpecs;
				}
			}
		}
	...
	return {
		...
		filterOfferByFeeType 	: _filterOfferByFeeType,
		isDynamicStored         : _isDynamicStored
	};
})();