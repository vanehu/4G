	...
	var _form_custInfomodify_btn = function() {
		...
		$('#custModifyForm').off("formIsValid").on("formIsValid",function(event) {
			...
			modifyCustInfo = {
					...
					mailAddressStr: $.trim($("#cmMailAddressStr").val())
				};
			var data = {};
			data.boCustInfos = [{
				...
				mailAddressStr :$("#boCustIdentities").attr("mailAddressStr"),
				state : "DEL"
			},{
				areaId : OrderInfo.cust.areaId,
				...
				mailAddressStr :modifyCustInfo.mailAddressStr,
				state : "ADD"
			}];
...
*/			        state : "DEL"//状态
	...
	var _identidiesTypeCdChoose = function(scope) {
		...
		}else if(identidiesTypeCd==6){
			$("#cmCustIdCard").attr("placeHolder","请输入合法证件号码");
			$("#cmCustIdCard").attr("data-validate","validate(digits:请准确填写证件号码) on(blur)");
		}else{
			$("#cmCustIdCard").attr("placeHolder","请输入合法证件号码");
		...
		var isIdTypeOff = $("#_session_staff_info").attr("idType")=="OFF";
...
/*		if(lteFlag){
	...
	var _showPasswordReset = function () {
		OrderInfo.busitypeflag=0;
		OrderInfo.initData(CONST.ACTION_CLASS_CD.PROD_ACTION,CONST.BO_ACTION_TYPE.PRODUCT_PASSWORD_RESET,0,CONST.getBoActionTypeName(CONST.BO_ACTION_TYPE.PRODUCT_PASSWORD_RESET),"");
		var param = _getCallRuleParam(CONST.BO_ACTION_TYPE.PRODUCT_PASSWORD_RESET,_choosedProdInfo.prodInstId);
		var callParam = {
			boActionTypeCd : CONST.BO_ACTION_TYPE.PRODUCT_PASSWORD_RESET,
			boActionTypeName : CONST.getBoActionTypeName(CONST.BO_ACTION_TYPE.PRODUCT_PASSWORD_RESET),
			accessNumber : _choosedProdInfo.accNbr,
			prodOfferName : _choosedProdInfo.prodOfferName
		};
...
/*		if(lteFlag){
	...
	//拆群
	var _showRemoveProdGroup = function () {
		OrderInfo.busitypeflag=40;
		if(!query.offer.setOffer()){
			return;
		}
		if(_isGroupMembers()){
			_removeCommit(CONST.PROD_STATUS_CD.REMOVE_PROD,CONST.BO_ACTION_TYPE.REMOVE_PROD,40);
		}
	};
	
	//判断是否有群成员
	var _isGroupMembers=function(){
		var param = {
				"accessNbr":_choosedProdInfo.accNbr,
				"curPage":1,
				"queryType":_choosedProdInfo.productId,
				"areaId":"",
				"pageSize":10
		};
		var url=contextPath+"/offer/getGroupInfoList";
		$.ecOverlay("<strong>正在查询中,请稍等会儿....</strong>");
			...
			if(response.data){
				if(response.data.list){
					if(response.data.totalSize==0){
						...
						$.alert("提示","存在群成员，不允许拆群!");
						return false;
					}
				}
			}
		}else {
			$.alert("提示","查询群详细信息失败,稍后重试");
			return false;
		}
		return false;
	};
