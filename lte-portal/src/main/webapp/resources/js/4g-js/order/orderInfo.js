...
OrderInfo = (function() {
	 ...
	 * 
	 * 31改产品密码，32重置产品密码，,33改产品属性，34修改短号，40拆群，41 新建客户
	 * 
	 */
	var _actionFlag = 0;
	...
	var _createAcct = function(busiOrders,acctId) {
		...
		var busiOrder = {
			areaId : OrderInfo.getAcctAreaId(),  //受理地区ID
			busiOrderInfo : {
				seq : OrderInfo.SEQ.seq--
			}, 
			busiObj : { //业务对象节点
			...
			data : {
				...
				boAccountItems : [], //账户属性节点
				boAccountMailings : [] //账单投递信息节点
			}
		};
		var boAccountItem_creditLimit = {
				itemSpecId : CONST.ACCT_ATTR.CREDIT_LIMIT,  //账户属性：信用额度 - 规格ID
				value : $.trim($("#creditLimit").val())*100,
				state : "ADD"	
		};
		busiOrder.data.boAccountItems.push(boAccountItem_creditLimit);
		if($("#postType").val()!=-1){
	...
	var _getAreaId = function(){
		...
		else if(OrderInfo.actionFlag==1 || OrderInfo.actionFlag==14 || OrderInfo.actionFlag==4){
			if(ec.util.isObj(OrderInfo.cust.areaId)){
	...
	var _getAcctAreaId = function(){
		...
		}else if(areaId==undefined || areaId==""){
			$.alert("错误提示","产品信息未返回地区ID，请营业后台核实！");				
			return;
		}
		return areaId;
	...
	var _isGroupProSpecId=function(specId){
		for(var i=0;i<CONST.GROUP_PROD_SPEC_ID.length;i++){
			if(CONST.GROUP_PROD_SPEC_ID[i]==specId){
				return true;
			}
		}
		return false;
	...
	//是否是群附属销售品
	var _isGroupOfferSpecId=function(offerSpecId){
		for(var i=0;i<CONST.GROUP_OFFER_SPEC_ID.length;i++){
			if(CONST.GROUP_OFFER_SPEC_ID[i]==offerSpecId){
				return true;
			}
		}
		return false;
