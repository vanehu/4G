...
//		SoOrder.step(1); //显示填单界面
	...
	var _submitOrder = function(data) {
		...
		if(OrderInfo.actionFlag==13){
			if (OrderInfo.cust.custId != undefined && OrderInfo.cust.custId != "") {
				data.coupons[0].partyId = OrderInfo.cust.custId;
			}
		}
		if(_getOrderInfo(data)){
			...
			if(OrderInfo.order.token!=""){
				url = contextPath+"/agent/order/orderSubmit?token="+OrderInfo.order.token;
			}
			if(OrderInfo.actionFlag==8 || OrderInfo.actionFlag==4){//实名制客户新建，和客户修改走一点提交接口
				url = contextPath+"/agent/order/orderSubmitComplete?token="+OrderInfo.order.token;
			}
...
//				}
		...
		if(OrderInfo.actionFlag==1 || OrderInfo.actionFlag==14){ //新装
			...
			if(OrderInfo.jbr.custId < -1 && OrderInfo.jbr.custId != OrderInfo.cust.custId){
	...
	var _orderConfirm = function(data){
		...
		if(OrderInfo.actionFlag==1){ //新装
			...
			$.each(OrderInfo.offerSpec.offerRoles,function(){
				$.each(this.prodInsts,function(){
					if(OrderInfo.getProdAn(this.prodInstId).accessNumber != undefined){
			...
			$("#order_prepare").hide();
			$("#orderTbody").append('<li class="list-group-item" id="offerSpecName"> <h4 class="list-group-item-heading"> 终端名称</h4><p class="list-group-item-text">'+OrderInfo.businessName+'</p></li>');
			...
			if (OrderInfo.actionFlag==13) {
				...
				if(bo2Coupons[0].couponInstanceNumber!=undefined){
					$("#orderTbody").append('<li class="list-group-item" id="offerSpecName"> <h4 class="list-group-item-heading"> 终端串码</h4><p class="list-group-item-text">'+bo2Coupons[0].couponInstanceNumber+'</p></li>');
				}
...
//				});
	...
	var _delOrderBegin = function(){
		...
		$(window).off("unload").on("unload", function(){
			var saveOrderSubmitStatus = $("#saveOrderSubmitStatus").val();
			if(saveOrderSubmitStatus=="1"){
				return;
			}
			SoOrder.delOrderSilent();
		});
...
//		var acctId =$("#acctSelect").val();
		...
		for ( var i = 0; i < busiOrder.data.ooRoles.length; i++) {
			...
			if(ooRole.objType==2){
				var accNbr = OrderInfo.getAccessNumber(ooRole.objInstId);
				if(accNbr != ""){
...
//					}
	...
	var _createMainOffer = function(busiOrders) {
		...
		$.each(OrderInfo.offerSpec.offerRoles,function(){
			$.each(this.prodInsts,function(){
			  i++;
			  if(memberRoleCd = 401 && i <= $('#ul_memeber').find("a").length+1){
				var ooRoles = {
...
//			}
			//校验号码跟UIM卡
			if($("#nbr_btn_-1").val()==undefined || $("#nbr_btn_-1").val() == ""){
				$.alert("信息提示","号码不能为空！");
				return false;
			} 
			if($("#uim_txt_-1").val()==undefined || $("#uim_txt_-1").val() == ""){
				$.alert("信息提示","UIM卡不能为空！");
				return false;
			} 
			if($("#tip_-1").text() != "校验通过"){
...
//					});
	...
	//判断选择的是纸质回执还是电子回执
	var _chooseReceiptType=function(type){
//		var receipType = $('input[name="buytype"]:checked ').val();
		var receipType = type;
		if(receipType=="2"){
			$("#div_toComplate").hide();
			$("#div_toTemporary").show();	
		}else{
			$("#div_toComplate").show();
			$("#div_toTemporary").hide();
		}
	...
	//生成二维码
	var _getRQCode = function(params){
	...
	return {
		...
		chooseReceiptType       :  _chooseReceiptType,
		getRQCode               :   _getRQCode
	};
})();