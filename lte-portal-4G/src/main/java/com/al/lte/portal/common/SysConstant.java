...
public final class SysConstant {
	...
	/** 问卷调查的系统id **/
	public static final String QESTION_SYS_ID = "102";
	...
	public static String[] itemType = { "未获取到订单类型", "新装", "套餐变更", "主副卡成员变更", "产品属性变更", "挂失/解挂", "停机保号/复机", "预拆机", "拆机",
			"违章拆机", "未激活拆机", "欠费拆机", "改客户资料返档", "补换卡", "可选包退订/订购", "过户", "改账务定制关系", "改产品密码" };
	...
	/**
	 * 过滤器过滤级别，默认为1；0：不过滤；1：登录后员工必须有渠道；2：替换提交到服务器端的敏感信息（staffId，staffCode等）；3：对需要鉴权的访问地址进行鉴权
	 * ；4：屏蔽报文（value）中包含已配置关键字的访问
	 */
	...
	/**
	 * 一证五卡文件类型标识 跨省一证五号图片：E4 跨省一证五号pdf：F 跨省一五身份证正面照：E1 跨省一五身份证背面照：E2
	 * 跨省一五经办人照片：E3 跨省一五其他：E
	 */
	...
	/**
	 * 跨省一证五号附件类型定义 跨省一证五号pdf：Pdf 跨省一五身份证正面照：Front 跨省一五身份证背面照：Back 跨省一五经办人照片：Jbr
	 * 跨省一五其他：Other
	 */
	...
	/**
	 * 图片批量上传,srcFlag（来源标识）取值定义 不传默认为WEB无纸化。 苹果直营店图片暂存需传“APPLE”; 实名制拍照留存需传“REAL”
	 */
