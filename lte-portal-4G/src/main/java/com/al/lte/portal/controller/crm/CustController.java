...
public class CustController extends BaseController {
	...
	public String queryCust(@RequestBody Map<String, Object> paramMap, Model model, @LogOperatorAnn String flowNum,
		...
		try {
			// 记录表SP_BUSI_RUN_LOG
			String custlogflag = MySimulateData.getInstance().getParam(
			...
			if ("ON".equals(custlogflag)) {
				...
				logmap.put("IDENTITY_NUM",
						(String) (paramMap.get("acctNbr") == "" ? paramMap.get("identityNum") == ""
								? paramMap.get("queryTypeValue") : paramMap.get("identityNum")
								: paramMap.get("acctNbr")));
			...
			if ("local".equals(diffPlace)) {// 如果是本地业务，判断传过来的地区是不是受理地区
				try {
					...
					if (!"0".equals(isCheckFlag)) {
						// 记录日志
						// 记录表SP_BUSI_RUN_LOG
						String custlogflag = MySimulateData.getInstance().getParam((String) ServletUtils
								.getSessionAttribute(super.getRequest(), SysConstant.SESSION_DATASOURCE_KEY),
								"CUSTLOGFLAG");
						if ("ON".equals(custlogflag)) {
							Map<String, Object> logmap = new HashMap<String, Object>();
							logmap.put("STAFF_CODE", sessionStaff.getStaffCode());
		 ...
		 * if(diffPlace.equals("local")){ param.put("areaId",
		 * sessionStaff.getCurrentAreaId()); }
		 */
		// 客户鉴权跳过 权限查询
		// String iseditOperation=
		...
		String DiffPlaceFlag = (String) (httpSession.getAttribute(SysConstant.SESSION_KEY_DIFFPLACEFLAG) == null ? ""
				: httpSession.getAttribute(SysConstant.SESSION_KEY_DIFFPLACEFLAG));
		...
		try {
			...
			if (MapUtils.isNotEmpty(resultMap)) {
				...
				if (!MapUtils.getString(paramMap, "queryTypeValue", "").equals("")
						&& !MapUtils.getString(paramMap, "queryType", "").equals("")) {
				...
				if (custInfos.size() > 0) {
					...
					Map<String, Map> listCustInfos = (Map<String, Map>) httpSession
							.getAttribute(SysConstant.SESSION_LIST_CUST_INFOS);
					...
					ServletUtils.setSessionAttribute(super.getRequest(),
							SysConstant.SESSION_KEY_AREA + "_" + sessionStaff.getStaffId(), custInfo.get("areaId"));
					...
					// 增加客户定位的证件标识用于星级服务
					if ("".equals(String.valueOf(paramMap.get("identityCd")))
							&& "".equals(String.valueOf(paramMap.get("queryType")))) {// 表示接入号定位
					...
					if (custInfos.size() >= 1) {
						...
						if (!(custInfos.size() == 1 && StringUtils.isBlank(identityCd)
							...
							if (!MapUtils.isNotEmpty(accNbrResultMap)) {
								...
								// (paramMap.get("acctNbr")==""?paramMap.get("identityNum")==""?paramMap.get("queryTypeValue"):paramMap.get("identityNum"):paramMap.get("acctNbr"));
								request.getSession().setAttribute("checkNumber", checkNumber);
							} else {
								request.getSession().removeAttribute("accNbrInfos");
								request.getSession().removeAttribute("checkNumber");
						...
						if (MapUtils.isNotEmpty(accNbrResultMap)) {
							...
							List<Map<String, Object>> accNbrCustInfos = (List<Map<String, Object>>) accNbrResultMap
									.get("custInfos");
						...
						PageModel<Map<String, Object>> pm = PageUtil.buildPageModel(1, 10, custInfosWithNbr.size(),
								custInfosWithNbr);
					...
					/*
					 * int count = (Integer)
					 * httpSession.getAttribute(sessionStaff.getStaffCode()+
					 * "custcount")+10;
					 * httpSession.setAttribute(sessionStaff.getStaffCode()+
					 * "custcount", count);
					 */
				}
			...
			if (null != paramMap.get("pageType") && !("").equals(paramMap.get("pageType"))
					&& ("orderUndo").equals(paramMap.get("pageType"))) {
	 ...
	 * @throws Exception
	 */
	private Map<String, Object> addAccountAndCustInfo(String flowNum, SessionStaff sessionStaff, String areaId,
			String custId, String accNbr, String soNbr, String identityCd) throws BusinessException, Exception {
	...
	public String queryCustSub(@RequestBody Map<String, Object> paramMap, Model model, @LogOperatorAnn String flowNum,
		...
		try {
			...
			if (beginTime != 0) {
				...
				long limit_time = Long.parseLong(MySimulateData.getInstance().getParam((String) ServletUtils
						.getSessionAttribute(super.getRequest(), SysConstant.SESSION_DATASOURCE_KEY), "LIMIT_TIME"));
				int limit_count = Integer.parseInt(MySimulateData.getInstance().getParam((String) ServletUtils
						.getSessionAttribute(super.getRequest(), SysConstant.SESSION_DATASOURCE_KEY), "LIMIT_COUNT"));
	...
	public void addSessionFlag(@RequestBody Map<String, Object> param, Model model, HttpSession httpSession) {
		try {
			...
			if ("cust".equals(flag)) {
				...
				String sessionCertNumber = (String) ServletUtils.getSessionAttribute(getRequest(),
						Const.CACHE_CERTINFO);
	...
	public String custAuth(@RequestBody Map<String, Object> param, Model model, @LogOperatorAnn String flowNum,
		...
		if ("0".equals(authFlag)) {
			if ("15678".contains(validateType)) {// 兼容客户定位省份证定位和二次业务证件类型定位
				...
				try {
					...
					checkIdMap.put("idCardNumber",
							StringUtils.isNotBlank(identityNum) ? Base64.eryDecoder(identityNum) : "");
					...
					Map<String, Object> datamap = mktResBmo.checkIdCardNumber(checkIdMap, flowNum, sessionStaff);
					if (datamap != null) {
			...
			String canJump = (String) ServletUtils.getSessionAttribute(super.getRequest(),
					SysConstant.SESSION_KEY_JUMPAUTH + "_" + sessionStaff.getStaffId());
			String custId = MapUtils.getString(param, "custId", "");
			boolean isSms = false;
			String smspwd = MapUtils.getString(param, "smspwd", "");
			String number = MapUtils.getString(param, "number", "");
			String smsPwdSession = (String) ServletUtils.getSessionAttribute(super.getRequest(),
					SysConstant.SESSION_KEY_CHANGEUIM_SMS);
			String numberSession = (String) ServletUtils.getSessionAttribute(super.getRequest(),
					"_custauth_sms_munber");// 客户鉴权短信验证号码
		...
		String custId = MapUtils.getString(param, "custId", "");
		if (SysConstant.ON.equals(sessionStaff.getPoingtType()) && !StringUtil.isEmptyStr(custId)
				&& !StringUtil.isEmpty(sessionStaff.getInPhoneNum())) {// 星级服务开关打开
			...
			try {
				...
				if (ResultCode.R_SUCC.equals(code) && returnMap.get("custInfo") != null
						&& returnMap.get("membershipLevelInfo") != null && returnMap.get("pointInfo") != null) {
	...
	public String custAuthSub(@RequestBody Map<String, Object> param, Model model, @LogOperatorAnn String flowNum,
	...
	public String custCreate(@RequestBody Map<String, Object> param, Model model, @LogOperatorAnn String flowNum,
			HttpServletResponse response, HttpSession httpSession) throws BusinessException {
		Map map = new HashMap();
		Map resultMap = new HashMap();
	...
	public String orderprod(@RequestParam Map<String, Object> param, Model model, HttpSession session,
		...
		// int pageSize =
		// sessionStaff.getAgentTypeCd().equals(SysConstant.AGENT_TYPE_IPAD) ? 8
		// : 12;
		String pageSize = "";
		...
		try {
			...
			if (ResultCode.R_SUCC.equals(code)) {
				...
				if (list == null) {
					...
					if (list.size() > 0) {
						ServletUtils.setSessionAttribute(super.getRequest(),
								SysConstant.SESSION_KEY_AREA + "_" + sessionStaff.getStaffId(),
								list.get(0).get("areaId"));
					}
					...
					PageModel<Map<String, Object>> pm = PageUtil.buildPageModel(
							...
							MapUtils.getIntValue(mktPageInfo, "totalCount", 1), list);
					model.addAttribute("pageModel", pm);
					model.addAttribute("custflag", "0");
					/*
					 * PageModel<Map<String, Object>> pm =
					 * PageUtil.buildPageModel(MapUtils.getIntValue(param,
	...
	public JsonResponse offerorderprod(@RequestBody Map<String, Object> param, @LogOperatorAnn String flowNum,
		...
		paramMap.put("pageSize", pageSize);
		try {
			datamap = this.custBmo.queryCustProd(paramMap, flowNum, sessionStaff);
			String code = (String) datamap.get("resultCode");
			if (ResultCode.R_SUCC.equals(code)) {
				Map<String, Object> temMap = (Map) datamap.get("result");
				list = (List<Map<String, Object>>) temMap.get("prodInstInfos");
	...
	public String starQueryList(Model model, @RequestParam Map<String, Object> paramMap, @LogOperatorAnn String flowNum,
		...
		try {
			...
			if (MapUtils.isNotEmpty(resultMap)) {
				...
				if (custInfos.size() > 0) {
					Map custInfo = (Map) custInfos.get(0);
					String idCardNumber = (String) custInfo.get("idCardNumber");
					session.setAttribute(SysConstant.IDCARDNUMBER + String.valueOf(paramMap.get("acctNbr")),
							idCardNumber);
					...
					if (ResultCode.R_SUCC.equals(code) && returnMap.get("custInfo") != null
						...
						for (int i = 0; i < pointInfolist.size(); i++) {
							...
							if ("100300".equals(map.get("pointItemID")) || "100800".equals(map.get("pointItemID"))
									|| "100600".equals(map.get("pointItemID"))) {
								pointlist.add(pointInfolist.get(i));
							}
						}
						PageModel<Map<String, Object>> pm = PageUtil.buildPageModel(nowPage, pageSize,
								totalSize < 1 ? 1 : totalSize, pointlist);
						model.addAttribute("pageModel", pm);
	...
	public String starQueryHisList(Model model, @RequestBody Map<String, Object> paramMap,
		...
		if (StringUtils.isNotEmpty(String.valueOf(paramMap.get("queryTypeValue")))) {
			String idCardNumber = (String) session
					.getAttribute(SysConstant.IDCARDNUMBER + String.valueOf(paramMap.get("queryTypeValue")));
		...
		PageModel<Map<String, Object>> pm = PageUtil.buildPageModel(nowPage, pageSize, totalSize < 1 ? 1 : totalSize,
				pointHistory);
		model.addAttribute("pageModel", pm);
	...
	@ResponseBody
	public JsonResponse uploadCustCertificate(@RequestBody Map<String, Object> param, @LogOperatorAnn String flowNum,
			HttpServletResponse response) {
		SessionStaff sessionStaff = (SessionStaff) ServletUtils.getSessionAttribute(super.getRequest(),
				SysConstant.SESSION_KEY_LOGIN_STAFF);
	...
	@ResponseBody
	public JsonResponse preHandleCustCertificate(@RequestBody Map<String, Object> param, @LogOperatorAnn String flowNum,
			HttpServletResponse response) {
		SessionStaff sessionStaff = (SessionStaff) ServletUtils.getSessionAttribute(super.getRequest(),
				SysConstant.SESSION_KEY_LOGIN_STAFF);
		Map<String, Object> result = null;
		...
		if (param.get("photograph") != null && param.get("venderId") != null
				&& !"".equals(param.get("photograph").toString()) && !"".equals(param.get("venderId").toString())) {
			try {
				Map<String, Object> resultImage = custBmo.preHandleCustCertificate(param.get("photograph").toString(),
						param.get("venderId").toString());
	...
	@ResponseBody
	public JsonResponse preCheckCertNumberRel(@RequestBody Map<String, Object> paramMap,
		...
		try {
			...
			/*
			 * if (MapUtils.isNotEmpty(resultMap)) { jsonResponse =
			 * super.successed(resultMap); } else { jsonResponse =
			 * super.failed(resultMap,
			 * ResultConstant.SERVICE_RESULT_FAILTURE.getCode()); }
			 */
		} catch (InterfaceException e) {
	...
	public JsonResponse getSeq(@RequestBody Map<String, Object> paramMap, @LogOperatorAnn String flowNum) {
		...
		try {
			...
			if (ResultCode.R_SUCC.equals(resultMap.get("resultCode"))) {
				...
				jsonResponse = super.failed(resultMap.get("resultMsg"),
						ResultConstant.SERVICE_RESULT_FAILTURE.getCode());
			}
		} catch (InterfaceException e) {
	...
	@ResponseBody
	public JsonResponse checkCustCert(@RequestBody Map<String, Object> paramMap, @LogOperatorAnn String flowNum,
			HttpServletResponse response) {
		...
		List<Map<String, Object>> list = null;
		try {
			resMap = custBmo.checkCustCert(paramMap, flowNum, sessionStaff);
	 ...
	 * 
	 * @param resquestMap
	 *            : { prodId : "", //产品实例id acctNbr : "", //接入号 prodSpecId : "",
	 *            //产品规格id areaId : "" //地区id }
	 */
				...
				if (prodInstParams != null) {
					...
					if (prodInstParams != null && prodInstParams.size() > 0) {
						for (Map<String, Object> prodInstParam : prodInstParams) {
							if (prodInstParam != null && SysConstant.PROD_ITEM_SPEC_ID_USER
									.equals(prodInstParam.get("itemSpecId") + "")) {
					...
					if (StringUtils.isNotBlank(custId)) { // 返回属性值为空字符串时不查询客户详情
						...
						try {
							...
							if (ResultCode.R_SUCC.equals(datamap.get("resultCode"))) {
								...
								List<Map<String, Object>> identities = (List<Map<String, Object>>) partyList
										.get("identities");
								...
								jsonResponse = super.failed(responseMap.get("resultMsg"),
										ResultConstant.SERVICE_RESULT_FAILTURE.getCode());
							}
						} catch (BusinessException be) {
							return super.failed(be);
	...
	public @ResponseBody JsonResponse verify(@RequestBody Map<String, Object> reqMap, String optFlowNum,
		...
		Map<String, Object> contractRootMap = (Map<String, Object>) reqMap.get("ContractRoot");
		Map<String, Object> svcContMap = (Map<String, Object>) contractRootMap.get("SvcCont");
		Map<String, Object> paramsMap = (Map<String, Object>) svcContMap.get("params");
		...
		if ("ON".equals(MapUtils.getString(fzConfig, "PROVINCE_FACE_VERIFY_FLAG", ""))
				&& "OFF".equals(MapUtils.getString(fzConfig, "FACE_VERIFY_SWITCH", ""))) {
			appId = MDA.XJ_FACE_VERIFY_APP_ID_SECRET;
		}
		String encryptAppIdStr = AESUtil.encryptToString(MDA.FACE_VERIFY_APP_ID, appId);
		svcContMap.put("app_id", encryptAppIdStr);
		Map<String, Object> paramMaps = (Map<String, Object>) ServletUtils.getSessionAttribute(getRequest(),
				Const.CACHE_CERTINFO_PARAM);
		String image_idcard = "";
		if (paramMaps != null) {
		...
		// if(StringUtils.isEmpty(sessionStaff.getProvinceCode()) &&
		// sessionStaff.getCurrentAreaId() !=null){
		...
		try {
			...
			if (rMap != null && ResultCode.R_SUCCESS.equals(rMap.get("code") + "")) {
				if (ResultCode.R_SUCC.equals(rMap.get("result_code") + "")) {
					// Map<String, Object> fzConfig = (HashMap<String, Object>)
					// MDA.FACE_VERIFY_FLAG
					// .get("FACE_VERIFY_"
					// + sessionStaff.getCurrentAreaId()
					// .substring(0, 3));
				...
				if (rMap != null) {
					...
					jsonResponse = super.failed("人证平台无返回值，可能原因为 调用人平台出现错误 。", ResultConstant.FAILD.getCode());
				}
			}
		} catch (BusinessException be) {
	...
	@ResponseBody
	public JsonResponse queryProdInstStats(@RequestBody Map<String, Object> paramMap, @LogOperatorAnn String flowNum,
			HttpServletResponse response) {
		Map<String, Object> resMap = null;
		JsonResponse jsonResponse = null;
		SessionStaff sessionStaff = (SessionStaff) ServletUtils.getSessionAttribute(super.getRequest(),
	...
	@ResponseBody
	public JsonResponse queryMktActivityList(@RequestBody Map<String, Object> paramMap, @LogOperatorAnn String flowNum,
			HttpServletResponse response) {
		Map<String, Object> resMap = null;
		JsonResponse jsonResponse = null;
		SessionStaff sessionStaff = (SessionStaff) ServletUtils.getSessionAttribute(super.getRequest(),
	...
	@ResponseBody
	public JsonResponse saveMktContactResult(@RequestBody Map<String, Object> paramMap, @LogOperatorAnn String flowNum,
			HttpServletResponse response) {
		Map<String, Object> resMap = null;
		JsonResponse jsonResponse = null;
		SessionStaff sessionStaff = (SessionStaff) ServletUtils.getSessionAttribute(super.getRequest(),
