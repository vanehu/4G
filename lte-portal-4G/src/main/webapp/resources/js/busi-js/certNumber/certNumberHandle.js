    ...
    function createAttachmentHtml(files, title, type) {
        ...
        $.each(files, function () {
            if ($.inArray(this.picFlag, type.split(',')) != -1) {
                if (this.picFlag == 'F') {
                    $(attachmentHtml).append($("<div style='width: 800px; height:600px;'></div>").append("<embed style='max-width:100%;max-height:100%;' src='" + "data:application/pdf;base64," + this.orderInfo + "'/>"));
                } else {
