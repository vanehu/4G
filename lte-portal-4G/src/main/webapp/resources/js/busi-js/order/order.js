...
//			}
		...
		if(OrderInfo.preBefore.prcFlag != "Y"){
			...
			var callback = function () {
				...
				if (OrderInfo.actionFlag == 2) {  //套餐变更不做校验
                    order.cust.preCheckCertNumberRelQueryOnly("-1", order.cust.getCustInfo415());//调用一证五号校验接口获取已有的数量
	...
	var _opeSer = function(inParam){	
		...
		$.each(offerSpec.offerRoles,function(){
			...
			// 新装纳入老用户作为副卡分省开关控制（在MDA配置）
			if(offerRole.memberRoleCd=="401" && areaidflag!="" && areaidflag.net_vice_card=="0" && "ON" == offerChange.queryPortalProperties("OLDMEMBER_" + OrderInfo.staff.soAreaId.substring(0,3))){
				$.each(this.roleObjs,function(){
					if(this.objType == CONST.OBJ_TYPE.PROD){
						var max = this.maxQty<0?"不限制":this.maxQty;//主卡的最大数量
						var min = this.minQty;
