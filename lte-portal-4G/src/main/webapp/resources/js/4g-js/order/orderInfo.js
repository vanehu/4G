...
OrderInfo = (function() {
	...
	1 新装		2  套餐变更	3  主副卡成员变更		4  产品属性变更	5  挂失/解挂		6  停机保号/复机	7  预拆机
	8  拆机		9  违章拆机	10  未激活拆机 		11  欠费拆机		12  改客户资料返档13  补换卡		14  可选包退订/订购
	15 过户		16 改账务定制关系					17 改产品密码	18 撤单 			19 异地补换卡	20 预写白卡
	21  换卡		22 补卡		27 预装				23 终端换货		24 裸机销售		25 新增客户		26 群组产品的拆机
	28 主副卡互换39 新装返销	40 补卡返销			41 合约返销		42 挂失			43 解挂			44 停机保号
	45 复机		46 终端预约	47 终端预约取消		48 重置产品密码	49 过户返档		50 改号			51 一卡双号退订
	64 靓号调级
	*/
	var _busitypeflag = 0;
				...
				if(ec.util.isArray(prodServ.prodSpecParams)){
					...
					$.each(prodServ.prodSpecParams,function(){
						...
						if(prodServ.servSpecId == CONST.YZFservSpecId && feeType == CONST.PAY_TYPE.AFTER_PAY){
							...
							if (this.itemSpecId == CONST.YZFitemSpecId1 || this.itemSpecId == CONST.YZFitemSpecId2 || (this.itemSpecId == CONST.YZFitemSpecId3 && !yiPayItemFlag)
							 ||  (this.itemSpecId == CONST.YZFitemSpecId4 && "ON" != offerChange.queryPortalProperties("AGENT_" + OrderInfo.staff.soAreaId.substring(0,3))) ) {
								this.setValue = "";
							}
						}
