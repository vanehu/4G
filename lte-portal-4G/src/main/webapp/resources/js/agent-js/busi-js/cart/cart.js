...
cart.main = (function(){
	...
	var _queryCartList = function(pageIndex,scroller){
		...
		if($("#if_p_olNbr").is(':checked')){
			...
			var startDt = $("#p_start_input").val().replace(/-/g,'');
			var endDt = $("#p_end_input").val().replace(/-/g,'');
			if(startDt>endDt){
				...
				param = {
						...
						"endDt":($("#p_end_input").val()).replace(/-/g,''),
						"qryNumber":qryNumber,
...
//			}
		...
		$.callServiceAsHtmlGet(contextPath+"/agent/report/cartList",param,{
			"before":function(){
	...
	//查询
	var _rqqueryCartList = function(pageIndex,scroller){
		OrderInfo.actionFlag=40;
		var curPage = 1 ;
		if(pageIndex>0){
			curPage = pageIndex ;
...
//			}
		...
		$.callServiceAsHtmlGet(contextPath+"/agent/report/rqCartList",param,{
			"before":function(){
				$.ecOverlay("购物车查询中，请稍等...");
			},
			"always":function(){
	...
	//滚动页面入口
	var _rqscroll = function(scrollObj){
		if(scrollObj && scrollObj.page && scrollObj.page >= 1){
			_rqqueryCartList(scrollObj.page,scrollObj.scroll);
		}
	};
	...
	var _queryCartInfo=function(olId){
		var param = {"olId":olId};
		param.areaId=$("#p_areaId").val();
		$.callServiceAsHtmlGet(contextPath+"/agent/report/cartInfo",param,{
			"before":function(){
				$.ecOverlay("详情查询中，请稍等...");
			},
			"always":function(){
	...
	var _initDic = function(){
		...
		$.callServiceAsJson(contextPath+"/agent/staffMgr/getCTGMainData",param,{
			"done" : function(response){
				if(response.code==0){
