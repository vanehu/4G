...
prod.changeUim = (function() {
	...
	var _initAgent = function(){
		...
		OrderInfo.initData(CONST.ACTION_CLASS_CD.PROD_ACTION,areaAtionType,actionFlag,opeName,"");
		rule.rule.prepareAgent(param,'prod.changeUim.initFillPageAgent',callParam);
		order.dealer.initDealer();
	};
	
	//初始化填单页面
	var _initFillPageAgent = function(param){
		if(!prod.uim.setProdUim()){ //根据UIM类型，设置产品是3G还是4G，并且保存旧卡
			return false;
		}
		var prodInfo = order.prodModify.choosedProdInfo;
		$.callServiceAsHtml(contextPath+"/agent/prodModify/changeCard", param, {
			"before":function(){
			},
	...
	return {
		...
		initAgent        : _initAgent,
		initFillPageAgent        : _initFillPageAgent
	};
})();