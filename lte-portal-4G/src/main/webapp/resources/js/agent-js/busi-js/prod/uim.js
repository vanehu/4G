...
//		var phoneNumber = $.trim($("#nbr_btn_"+prodId).val()); 
		var phoneNumber = OrderInfo.getAccessNumber(prodId);
		var offerId = "-1"; //新装默认，主销售品ID
		if(OrderInfo.actionFlag==1){
			phoneNumber = $.trim($("#nbr_btn_"+prodId).val());
			if(phoneNumber==''){
				$.alert("提示","校验UIM卡前请先选号!");
				return false;
			}
		}
		if(OrderInfo.actionFlag==6||OrderInfo.actionFlag==14){ //新装需要选号
			if(phoneNumber==''){
				$.alert("提示","校验UIM卡前请先选号!");
				return false;
			}
		...
		if(cardNo==undefined || cardNo=='' || cardNo.length > 25){
			$.alert("提示","请正确填写UIM卡号!");
			return false;
		}
		var inParam = {
		...
		//zhengyx7 begin
		$("#isCheck_"+prodId).css("display","initial");
		// end
		...
		if ("1" == OrderInfo.actionFlag) {
			$("#uim_release_btn_"+prodId).attr("disabled",false);
			$("#uim_scann_btn_"+prodId).attr("disabled","disabled");
		}
		//
	...
	//购手机uim卡号校验
	var _checkUimPhone = function(prodId){
		var phoneNumber = OrderInfo.getAccessNumber(prodId);
		var offerId = "-1"; //新装默认，主销售品ID
...
//				inParam.mktResCd = mktResCd;
		...
		var coupon = {
			...
			agentId : 1, //供应商ID
			apCharge : 0, //物品价格
			couponInstanceNumber : data.baseInfo.mktResInstCode, //物品实例编码$("#tsn").val()
			terminalCode : data.baseInfo.mktResInstCode,//前台内部使用的UIM卡号
			ruleId : "", //物品规则ID
			partyId : OrderInfo.cust.custId, //客户ID
			prodId :  prodId, //产品ID
		...
		$("#uim_release_btn_"+prodId).attr("disabled",false);
		$("#sm_"+prodId).attr("disabled",true);
//		$("#uim_release_btn_"+prodId).removeClass("disabled");
		$("#uim_txt_"+prodId).attr("disabled",true);
		//选号选套餐按钮不可用
		$("#gouhyj").attr("disabled",true);
		$("#goulj").attr("disabled",true);
		$("#hyxh").attr("disabled",true);
	...
	var getMktResCd = function(prodSpecId){
		...
		var url = contextPath+"/agent/mktRes/uim/getMktResCd";
...
/*		var phoneNumber = OrderInfo.getAccessNumber(prodId);
		...
		var jr = $.callServiceAsJson(contextPath+"/agent/mktRes/phonenumber/releaseErrorNum", param);			
		if(jr.code==-2){
			$.alertM(jr.data);
			return;
		}
		...
		if(OrderInfo.actionFlag==1){
			...
			$("#uim_scann_btn_"+prodId).attr("disabled","");
		}
		$("#tip_"+prodId).text("");
...
//		$("#uim_release_btn_"+prodId).removeClass("disabled");
		...
		if(OrderInfo.actionFlag==22 && data.baseInfo.cardTypeFlag==1){
		//	_queryAttachOffer();
		  AttachOffer.queryCardAttachOfferAgent(data.baseInfo.cardTypeFlag);  //加载附属销售品
	    }
	...
	//
	//uim卡号释放
	var _releaseUimAgent = function(prodId){	
		var cardNo =$.trim($("#uim_txt_"+prodId).val());
		if(cardNo==undefined || cardNo==''){
			$.alert("提示","UIM卡不能为空!");
			return false;
