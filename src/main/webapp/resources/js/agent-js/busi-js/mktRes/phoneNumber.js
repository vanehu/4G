...
order.phoneNumber = (function(){
	...
	var idcode=[];
	//请求地址
	var url = contextPath+"/agent/mktRes/phonenumber/list";
	var phoneNumberVal="";
	//按钮查询
	...
	var _queryPhoneNumber=function(param){
		var url = contextPath+"/agent/mktRes/phone/numberlist";
		//收集参数
		var params = {
...
//	};
	...
	var queryPhoneNbrPool = function(){
		...
		var url=contextPath+"/agent/mktRes/phonenumber/queryPhoneNbrPool";
		var param={};
...
//				$sel.addClass("styled-select");
	...
	var _endSelectNum=function(obj,purchas){
		...
		if ("ON" == numberCheckFlag) {
			...
			var response = $.callServiceAsJson(contextPath + "/agent/prodModify/preCheckBeforeOrde", {"serviceType": 38, "accNbr": accNbr});
			if(response.code == 1){
