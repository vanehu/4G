...
cart.main = (function() {
    ...
    var _queryCartList = function(pageIndex, scroller) {
        ...
        if ($("#if_p_olNbr").is(':checked')) {
           ...
           // cart.main.queryType="";
            if(cart.main.queryType=="tj"){//统计走新的接口查询
            	_queryTjList(param,curPage);
            	return;
            }
        }
        param.newFlag="1";
 ...
 //统计查询  
   var _queryTjList=function(param,curPage){
       $.callServiceAsHtmlGet(contextPath + "/app/report/cartListTj", param, {
           "before": function() {
               $.ecOverlay("购物车查询中，请稍等...");
           },
           "always": function() {
...
//					OrderInfo.handleInfo.staffList = response.data;
  ...
  var _closeCartInfo=function(){
	  $("#cart_info_modal").modal("hide");
  };
  ...
  var _toggleTabBtn = function(){
  	if($("#nav-tab-2").hasClass("active in")){
    	...
    	$("#p_startDt").val($("#initDate").val());
    	$("#p_startDt").trigger("change")
    	$("#p_olStatusCd").val("");
    	$("#p_olStatusCd").trigger("change");
    	$("#p_qryNumber").val("");
	}
