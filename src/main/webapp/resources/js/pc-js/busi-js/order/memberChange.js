...
//			order.memberChange.closeDialog();
		...
		if(num>0){
			...
			if((parseInt(num)+parseInt(ec.util.mapGet(OrderInfo.oneCardFiveNO.usedNum,order.cust.getCustInfo415Flag(order.cust.getCustInfo415()))))>5){
				...
				var tips = "每个证件在全国范围最多可加装5张移动号卡，您当前业务在本证件下新增"+num+"张号卡，合计超过5卡，请对于超出的号卡登记其他使用人";
	        	if(usedNum>0){
	        		tips = "每个证件在全国范围最多可加装5张移动号卡，此用户下已经有"+usedNum+"个号码，您当前业务在本证件下新增"+num+"张号卡，合计超过5卡，请对于超出的号卡登记其他使用人";
	        	}
	            $.alert("提示",tips);
	        }
		}
