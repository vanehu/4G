		...
		if(OrderInfo.actionFlag==1 || OrderInfo.actionFlag==14){ //新装
			...
			if(OrderInfo.newClothes=="true"  || OrderInfo.oldMember=="true"){
				...
				if(OrderInfo.delViceCard=="true"){
					viceparam=OrderInfo.viceParam.viceParam;
					ooRoles=OrderInfo.viceParam.ooRoles;
...
//		var olId = OrderInfo.orderResult.olId;
	...
	var _delViceCard = function(busiOrders,data){
		...
		OrderInfo.getOfferBusiOrder(busiOrders,param,data[0].objInstId,null,2);
		$.each(data,function(){
			var prod = {
	...
	var _delViceCardAndNew = function(busiOrders,data){		
		...
		OrderInfo.getOfferBusiOrder(busiOrders,param,objInstId,null,2);		
		//订购副卡主套餐
		for (var i = 0; i < newData.length; i++) {
			...
			busiOrders.push(OrderInfo.getProdBusiOrder(prod,2));
			}
			if(order.memberChange.getSimulateData()){
						...
						if(ec.util.isArray(OrderInfo.offer.offerMemberInfos)){ //遍历主销售品构成
							$.each(OrderInfo.offer.offerMemberInfos,function(){
								if(prodId==this.objInstId&&this.objType==CONST.OBJ_TYPE.PROD && this.prodClass==CONST.PROD_CLASS.THREE && OrderInfo.offerSpec.is3G=="N"){//补换卡
									if(AttachOffer.isChangeUim(this.objInstId,"delViceCard")&&(OrderInfo.boProd2Tds.length>0||OrderInfo.zcd_privilege==0)){
										var prod = {
												prodId : this.objInstId,
												prodSpecId : this.objId,
												accessNumber : this.accessNumber,
				...
				AttachOffer.setAttachBusiOrder(busiOrders,null,1);  //订购退订附属销售品
			}
		}
	};
...
//		var acctId =$("#acctSelect").val();
	...
	//创建主副卡订单数据
	var _createMainOrder = function(busiOrders,data) {
		...
		if(OrderInfo.oldMember=="true"){//纳入老成员
...
//					}
						...
						for ( var i = 0; i < OrderInfo.oldoffer.length; i++) { //遍历主销售品构成
							$.each(OrderInfo.oldoffer[i].offerMemberInfos,function(){
								if(this.objType==CONST.OBJ_TYPE.PROD && this.prodClass==CONST.PROD_CLASS.THREE && OrderInfo.offerSpec.is3G=="N"){//补换卡
									if(AttachOffer.isChangeUim(this.objInstId,"olemember")&&(OrderInfo.boProd2Tds.length>0||OrderInfo.zcd_privilege==0)){
										var prod = {
											prodId : this.objInstId,
											prodSpecId : this.objId,
											accessNumber : this.accessNumber,
			...
			//	AttachOffer.setAttachBusiOrder(busiOrders,"oldmember",1);//添加附属
			//	busiOrders.push(busiOrder);
		}
		
		//新装
		if(OrderInfo.newClothes=="true")
...
//	    		OrderInfo.getOfferBusiOrder(busiOrders,param,objInstId);
	...
	//创建附属销售品节点
	var _createAttOffer = function(offerSpec,prodId,flag,busiOrders,action,sign) {
		if(flag==1){  //退订附属
			offerSpec.offerTypeCd = 2;
			offerSpec.boActionTypeCd = CONST.BO_ACTION_TYPE.DEL_OFFER;
			OrderInfo.getOfferBusiOrder(busiOrders,offerSpec,prodId,action,sign);
		}else if(flag==2){  //参数变更
	...
	//创建功能产品节点
	var _createServ = function(servSpec,prodId,flag,busiOrders,action,sign) {
		servSpec.prodId = prodId;
		if(flag==1){  //退订附属
			servSpec.servClose = "Y";
			servSpec.boActionTypeCd = CONST.BO_ACTION_TYPE.SERV_OPEN;
			busiOrders.push(OrderInfo.getProdBusiOrder(servSpec,action,sign));	
		}else if(flag==2){  //参数变更
...
//			AttachOffer.changeLabel(prodId, prodInfo.productId,""); //初始化第一个标签附属
	...
	//删除附属销售品实例
	var _delOffer = function(prodId,offerId,reload){
		...
		if($span.attr("class")=="del"){  //已经退订，再订购
			...
			if(!ec.util.isArray(offer.offerMemberInfos)){	
				...
				if(OrderInfo.actionFlag==6){
	                  ...
	                  if(acctNbr==undefined || acctNbr=="" || acctNbr==null){
	                	  for(var i=0;i<OrderInfo.oldprodInstInfos.length;i++){
	  						if(prodId==OrderInfo.oldprodInstInfos[i].prodInstId){
			...
			if(OrderInfo.provinceInfo.reloadFlag=="N"){//如果是二次加载
				offer.isdel = "Y";
				$span.addClass("del");
...
//								}
	...
	//判断是否需要补卡
	var _isChangeUim = function(objId,action){
		if(CONST.getAppDesc()==0){
			var prodClass = order.prodModify.choosedProdInfo.prodClass; //可选包变更
			var prodId = order.prodModify.choosedProdInfo.prodInstId;
			if(OrderInfo.actionFlag==6){
...
//		var params={"qryStr":qryStr,"pnLevelId":"","custId":custId};
	...
	var _queryData = function(params,flag,scroller) {
		...
		var url = contextPath+"/token/app/order/offerSpecListSub";
		$.callServiceAsHtmlGet(url,params, {
			"before":function(){
...
//	        }
	...
	var _offerDialog=function(subPage){
		_subPage = subPage;
		var param={};
		var url=contextPath+"/token/app/order/prodoffer/preparesub?subPage="+subPage;
...
query.offer = (function() {
	...
	//套餐变更，查询附属销售品页面
	var _queryChangeAttachOffer = function(param,action,sign) {
		addParam(param,action,sign);  //添加基本参数
	...
	// 查询默认必须可选包
	var _queryDefMustOfferSpec = function(param,action,sign) {
		addParam(param,action,sign);  //添加基本参数
	...
	var _queryServSpec = function(param,action,sign) {
		addParam(param,action,sign);  //添加基本参数
	...
	var _queryMainOfferSpec = function(param){
		...
		if(OrderInfo.actionFlag==6 && OrderInfo.oldMember=="true"){//主副卡纳入老用户
	...
	//补充查询基本条件
	var addParam = function(param,action,sign){
		/**
		 * action 1:成员变更新装或者纳入老成员
		 *        2：拆卡或者拆卡变套餐
		 * sing newmember: 新装标识
		 *      oldmember: 纳入老成员
		 *      delViceCard: 拆副卡或者拆副卡变套餐 
		 * */
...
//		}
	...
	var _parseServ = function(data,prodId,action){
		if(ec.util.isObj(data)){
			if(ec.util.isArray(data.result.servSpec)){
				for (var i = 0; i < data.result.servSpec.length; i++) {
					...
					}else if((OrderInfo.actionFlag==6)&&servSpec.ifDault==1){//套餐变更需要展示默认的功能产品
						if(action=="delViceCard"){
						if(ec.util.isObj(serv)){
							var $dd = $("#li_"+prodId+"_"+serv.servId);
							if(ec.util.isObj($dd)){
