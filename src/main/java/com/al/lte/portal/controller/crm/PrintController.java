...
//				SysConstant.SESSION_KEY_LOGIN_STAFF);
    ...
    @RequestMapping(value = "/getEQCodeInfo", method = {RequestMethod.GET, RequestMethod.POST})
    @ResponseBody
    public JsonResponse getEQCodeInfo(Model model, @LogOperatorAnn String flowNum,@RequestBody Map<String, Object> param) {
    	...
    	try {
	    	...
	    	dataBusMap.put("olId", param.get("olId"));
	    	dataBusMap.put("areaId", sessionStaff.getCurrentAreaId().toString());
//		    String str=JacksonUtil.getInstance().objctTojson(map);
		    Map<String, Object> resultMap = this.printBmo.getEQCodeInfo(map,flowNum,sessionStaff);
		    resultCode = resultMap.get("code").toString();
