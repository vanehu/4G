...
//				String ifOrderAgain = MapUtils.getString(offerSpecCanBuy, "ifOrderAgain", "N/A");
	...
	public Map<String, Object> queryProdOfferOrderTimes(Map<String, Object> paramMap,
			 String optFlowNum, SessionStaff sessionStaff) throws Exception {
			DataBus db = InterfaceClient.callService(paramMap,
					PortalServiceCode.QUERY_PROD_OFFER_TIMES, optFlowNum, sessionStaff);
			Map<String, Object> resultMap = new HashMap<String, Object>();
			try{
				// 服务层调用与接口层调用都成功时，返回列表；否则返回空列表
				if (ResultCode.R_SUCC.equals(StringUtils.defaultString(db.getResultCode()))) {
				...
				log.error("门户处理营业后台的queryProdOfferOrderTimes服务返回的数据异常", e);
				throw new BusinessException(ErrorCode.QUERY_PROD_OFFER_TIMES, paramMap, resultMap, e);
			}
			return resultMap;
		}
}
