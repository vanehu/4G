...
order.dealer = (function() {
	...
	var _initDealer = function() {
		...
		if(OrderInfo.actionFlag==1 || OrderInfo.actionFlag==2 || OrderInfo.actionFlag==14){ //新装业务，套餐变更需要主套餐发展人
			...
			var $div2 = $('<div class="col-xs-6" style="padding-right: 0;"> </div>');
...
//				$dl.append($dd);
//			}else{
				var $td = $('<div class="col-xs-6"><input type="text" objId="'+objId+'" readonly="readonly" onclick="javascript:order.dealer.showDealer(0,\'dealer\');" class="form-control showfzr" id="dealer_'+objId+'" staffId="'+OrderInfo.staff.staffId+'" value="'+OrderInfo.staff.staffName+'" ></input></div>');
				$div.append($td);
//			}
			$li.append($div);
			$li.append($p);
...
//	    			}
			...
			if(_isChange){
				$("li[name^='tr_"+prodId+"']").each(function(){
					$(this).find("h5").eq(0).text(_accNbr);
				});
			}
		}
		...
		if(OrderInfo.actionFlag==21){//副卡套餐变更
			if(ec.util.isArray(OrderInfo.viceOfferSpec)){
				$.each(OrderInfo.viceOfferSpec,function(){
	    			...
	    			$li.append("<h5 class='list-group-item-heading text-warning'>"+accNbr+"</h5>");
	    			$li.append("<p class='list-group-item-text'>"+this.offerSpecName+"（包含接入产品）</p><p></p>");
	    			var $p = $('<p> </p>');
	    			var $div = $('<div class="row"> </div>');
	    			var $div2 = $('<div class="col-xs-6"> </div>');
	    						//var $field=$('<fieldset data-role="fieldcontain"></fieldset>');
...
//	    				$dl.append($dd);
//	    			}else{
	    				var $td = $('<div class="col-xs-6"><input type="text" readonly="readonly" onclick="javascript:order.dealer.showDealer(0,\'dealer\');" class="form-control" id="dealer_'+objId+'" staffId="'+OrderInfo.staff.staffId+'" value="'+OrderInfo.staff.staffName+'" ></input></div>');
	    				$div.append($td);
//	    			}
	    			$li.append($div);
	    			$li.append($p);
			...
			"done" : function(response){
				...
				if(response.code==0){
					if(response.data.length == 0){
						...
						$("#dealer_"+objInstId).val(response.data[0].staffName);
						$("#dealer_"+objInstId).attr("value",response.data[0].staffName);
						$("#dealer_"+objInstId).attr("staffId", response.data[0].staffId);
