...
common.print = (function($){
	...
	var _signVoucher=function(params){
		...
		var url=contextPath+"/order/sign/previewHtmlForAgentSign";
		$.callServiceAsHtml(url, params, {
			...
			"done" : function(response){
				if (response.code == 0) {
					...
					if(printStatus!="0"){
						//$("#datasign").attr("src",decodeURIComponent($("#imgData").val()));
						common.print.showDataSign($("#signinput").val());
						$("#datasignBtn").attr("disabled","disabled");
						$("#photoPrint").attr("disabled","disabled");
						$("#print_ok").attr("disabled","disabled");
					}
		...
		$.callServiceAsJson(url, params, {
			...
			"done" : function(response){
				...
				if (response.code == 0) {
                    ...
                    $("#printStatus").val("1");
                    //var imgData = encodeURIComponent($("#datasign").attr("src"));
                    //$("#imgData").val(imgData);
					$("#order-confirm").show();
					$("#order-print").hide();
					OrderInfo.order.step=3;
