	...
	//	alert("idnum:"+idnum);
		...
		for(var i=0;i<OrderInfo.oldAddNumList.length;i++){
			...
			if (response.code == 0) {
				...
				var custId = response.data.custInfos[0].custId;
				var offerInfos = {
					"custInfos":response.data.custInfos[0]
				};
				if(OrderInfo.oldCustInfos == undefined){
					OrderInfo.oldCustInfos = [];
				}
				OrderInfo.oldCustInfos.push(offerInfos);
				// 主副卡客户校验取客户查询返回结果中的custId,旧的取order.prodModify.choosedProdInfo.custId
...
//			order.memberChange.closeDialog();
		...
		if(num>0){
			...
			for(var i=0;i<OrderInfo.oldCustInfos.length;i++){
				if(OrderInfo.oldCustInfos[i].custInfos.identityCd!="1"){
					isRequired = "Y";
					break;
				}
			}
		}
		OrderInfo.order.isRequired = isRequired;
		if(OrderInfo.order.orderAttrFlag=="Y" && isRequired == "Y"){
			$.alert("提示","省份未拍照或者未传正确的经办人信息，当前业务场景经办人必填，无法继续办理业务，请省份确认经办人信息！");
			return;
		}
