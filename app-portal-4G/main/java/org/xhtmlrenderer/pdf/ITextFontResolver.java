...
public class ITextFontResolver implements FontResolver {
	...
	public FSFont resolveFont(SharedContext renderingContext,
		...
		return resolveFont(renderingContext, spec.families, spec.size,
	...
	public void flushFontFaceFonts() {
		...
		for (Iterator i = this._fontFamilies.values().iterator(); i.hasNext();) {
			...
			for (Iterator j = family.getFontDescriptions().iterator(); j
					.hasNext();) {
	...
	public void importFontFaces(List fontFaces) {
		for (Iterator i = fontFaces.iterator(); i.hasNext();) {
			...
			byte[] font1 = this._sharedContext.getUac().getBinaryResource(
					src.asString());
			if (font1 == null) {
			...
			byte[] font2 = null;
			FSDerivedValue metricsSrc = style
					.valueByName(CSSName.FS_FONT_METRIC_SRC);
			if (metricsSrc != IdentValue.NONE) {
				font2 = this._sharedContext.getUac().getBinaryResource(
						metricsSrc.asString());
				if (font2 == null) {
					XRLog.exception("Could not load font metric data "
							+ src.asString());
				}
			...
			boolean embedded = style.isIdent(CSSName.FS_PDF_FONT_EMBED,
					IdentValue.EMBED);

			String encoding = style
					.getStringProperty(CSSName.FS_PDF_FONT_ENCODING);
			try {
				addFontFaceFont(src.asString(), encoding, embedded, font1,
						font2);
	...
	private void addFontFaceFont(String uri, String encoding, boolean embedded,
			byte[] afmttf, byte[] pfb) throws DocumentException, IOException {
		String lower = uri.toLowerCase();
		if ((lower.endsWith(".otf")) || (lower.endsWith(".ttf"))
				|| (lower.indexOf(".ttc,") != -1)) {
			BaseFont font = BaseFont.createFont(uri, encoding, embedded, false,
					afmttf, pfb);
			...
			fontFamily.addFontDescription(descr);
				...
				|| (lower.endsWith(".pfb")) || (lower.endsWith(".pfa"))) {
			...
			BaseFont font = BaseFont.createFont(name, encoding, embedded,
					false, afmttf, pfb);
	...
	public FontFamily getFontFamily(String fontFamilyName) {
		FontFamily fontFamily = (FontFamily) this._fontFamilies
				.get(fontFamilyName);
		if (fontFamily == null) {
	...
	private FSFont resolveFont(SharedContext ctx, String[] families,
			float size, IdentValue weight, IdentValue style, IdentValue variant) {
		...
		if (families != null) {
			for (int i = 0; i < families.length; ++i) {
				FSFont font = resolveFont(ctx, families[i], size, weight,
						style, variant);
				if (font != null) {
					return font;
				}
	...
	private FSFont resolveFont(SharedContext ctx, String fontFamily,
			float size, IdentValue weight, IdentValue style, IdentValue variant) {
		String normalizedFontFamily = normalizeFontFamily(fontFamily);

		String cacheKey = getHashName(normalizedFontFamily, weight, style);
		FontDescription result = (FontDescription) this._fontCache
				.get(cacheKey);
		if (result != null) {
			return new ITextFSFont(result, size);
		}

		FontFamily family = (FontFamily) this._fontFamilies
	...
	protected static String getHashName(String name, IdentValue weight,
			IdentValue style) {
	...
	private static BaseFont createFont(String name) throws DocumentException,
			IOException {
		return BaseFont.createFont(name, "winansi", true);
	}

	private static void addCourier(HashMap result) throws DocumentException,
			IOException {
		FontFamily courier = new FontFamily();
		courier.setName("Courier");

		courier.addFontDescription(new FontDescription(
				createFont("Courier-BoldOblique"), IdentValue.OBLIQUE, 700));

		courier.addFontDescription(new FontDescription(
				createFont("Courier-Oblique"), IdentValue.OBLIQUE, 400));

		courier.addFontDescription(new FontDescription(
				createFont("Courier-Bold"), IdentValue.NORMAL, 700));

		courier.addFontDescription(new FontDescription(createFont("Courier"),
				IdentValue.NORMAL, 400));
	...
	private static void addTimes(HashMap result) throws DocumentException,
			IOException {
		FontFamily times = new FontFamily();
		times.setName("Times");

		times.addFontDescription(new FontDescription(
				createFont("Times-BoldItalic"), IdentValue.ITALIC, 700));

		times.addFontDescription(new FontDescription(
				createFont("Times-Italic"), IdentValue.ITALIC, 400));

		times.addFontDescription(new FontDescription(createFont("Times-Bold"),
				IdentValue.NORMAL, 700));

		times.addFontDescription(new FontDescription(createFont("Times-Roman"),
				IdentValue.NORMAL, 400));
	...
	private static void addHelvetica(HashMap result) throws DocumentException,
			IOException {
		FontFamily helvetica = new FontFamily();
		helvetica.setName("Helvetica");

		helvetica.addFontDescription(new FontDescription(
				createFont("Helvetica-BoldOblique"), IdentValue.OBLIQUE, 700));

		helvetica.addFontDescription(new FontDescription(
				createFont("Helvetica-Oblique"), IdentValue.OBLIQUE, 400));

		helvetica.addFontDescription(new FontDescription(
				createFont("Helvetica-Bold"), IdentValue.NORMAL, 700));

		helvetica.addFontDescription(new FontDescription(
				createFont("Helvetica"), IdentValue.NORMAL, 400));
	...
	public static class FontDescription {
		...
		private void setMetricDefaults() {
			...
			if (box != null) {
				...
				this._yStrikeoutPosition = (this._font.getFontDescriptor(8,
						1000.0F) / 3.0F);
				this._yStrikeoutSize = 100.0F;
			}
		}
	...
	private static class FontFamily {
		...
		public ITextFontResolver.FontDescription match(int desiredWeight,
				IdentValue style) {
			if (this._fontDescriptions == null) {
			...
			for (Iterator i = this._fontDescriptions.iterator(); i.hasNext();) {
				ITextFontResolver.FontDescription description = (ITextFontResolver.FontDescription) i
						.next();
			...
			ITextFontResolver.FontDescription[] matches = (ITextFontResolver.FontDescription[]) (ITextFontResolver.FontDescription[]) candidates
					.toArray(new ITextFontResolver.FontDescription[candidates
							.size()]);

			ITextFontResolver.FontDescription result = findByWeight(matches,
					desiredWeight, 1);
		...
		private ITextFontResolver.FontDescription findByWeight(
				ITextFontResolver.FontDescription[] matches, int desiredWeight,
				int searchMode) {
			if (searchMode == 1) {
