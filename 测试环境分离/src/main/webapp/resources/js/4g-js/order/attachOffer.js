...
//		SoOrder.initFillPage();
		...
		//redmine 1056887
		var prodMod = order.prodModify.choosedProdInfo ; 
 		var paramQry = {
 			offerId : prodMod.prodOfferInstId,
 			offerSpecId : prodMod.prodOfferId,
 			areaId : prodMod.areaId,
 			distributorId : ""
 		};
 		// 安全办公销售品构成查询兼容根据bizId查询
 		if ("bizId" == order.cust.custQueryParam.queryType) {
 			paramQry.BIZID = prodMod.accNbr;
 			paramQry.flag = "all";
 		} else {
 			paramQry.acctNbr = prodMod.accNbr;
 		}
         var data = query.offer.queryOfferInst(paramQry); //查询销售品实例构成
       ...
       //如果objId，objType，objType不为空才可以查询默认必须
 		if(ec.util.isObj(member.objId)&&ec.util.isObj(member.objType)&&ec.util.isObj(member.offerRoleId)){
 			...
 			param.memberRoleCd = member.roleCd;
 			//默认必须可选包
 			var dataOfferSpec = query.offer.queryDefMustOfferSpec(param);
 			CacheData.parseOffer(dataOfferSpec);
 			//默认必须功能产品
 			var dataServSpec = query.offer.queryServSpec(param);
 			CacheData.parseServ(dataServSpec);
 		}
