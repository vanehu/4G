 ...
 * 反销账
 */
public class ReverseWriteOffCash extends Service {
	...
	public DataMap exec(DataMap dataMap, String serviceSerial) throws Exception {
		...
		String[] params = { "phoneNumber", "serialType", "serialNbr", "areaCode", "channelId", "staffId" };
...
//		dataMap.setOutParam(returnMap);
	   ...
	   @SuppressWarnings("unchecked")
	private void logRWriteOffCash(DataMap dataMap) throws Exception {
	        ...
	        logMap.put("PAY_AMOUNT", Long.parseLong("1"));
	        if(ResultConstant.R_POR_SUCCESS.getCode().equals(MapUtils.getString(dataMap.getOutParam(), "code", ""))){
	        	logMap.put("PAY_STATUS", LteConstants.CON_PAY_STATUS_SUCESS);
	        	Map<String, Object> result = MapUtils.getMap(dataMap.getOutParam(), "result", new HashMap<String, Object>());
	        	if(!result.isEmpty()){
	        		logMap.put("PAY_AMOUNT", MapUtils.getLongValue(result, "reverseAmount",-1)*(-1));
	        	}
	        }else{
	        ...
	        logMap.put("PAY_TYPE", LteConstants.CON_PAY_TYPE_R_WRITE_OFF_CASH);
