...
public class SoWriteCardDAOImpl implements SoWriteCardDAO {
    ...
    public Map<String, Object> getCardDllInfo(DataMap dataBus) throws Exception {
    	String factoryCode = (String) dataBus.getInParam("factoryCode");
        if (StringUtils.isBlank(factoryCode)) {
            throw new Exception("factoryCode is blank ");
        }
        String cardType = (String) dataBus.getInParam("cardType");
        ...
        if("newCard".equals(cardType)){
        	sql.append(" WHERE FLAG = 9");
        }else{
        ...
        Map<String, Object> map = jdbc.queryForMap(sql.toString(), new Object[] { factoryCode },
    ...
    public String getPasswordKey(String factoryCode, String keyType) throws Exception {
        ...
        List<String> list = jdbc.queryForList(sql.toString(), new Object[] { factoryCode, keyType }, String.class);
        if (list.isEmpty()) {
            return null;
        } else {
            return list.get(0);
    ...
    public String getDllKey(String factoryCode) throws Exception {
        ...
        List<String> list = jdbc.queryForList(sql.toString(), new String[] { factoryCode,
                WriteCardUtil.DLL_KEY_DECRIPT_KEY }, String.class);
        if (list.isEmpty()) {
            return null;
        } else {
            return list.get(0);
    ...
    private Object[] createInsertDepPrvncIntfLogSqlParams(Map params) {
        ...
        return new Object[] {
                ...
                MapUtils.getString(map, "ACTION_CODE"), MapUtils.getString(map, "BUS_CODE"),
                MapUtils.getString(map, "SVC_CODE"),
                MapUtils.getString(map, "STATE"),//not null
                MapUtils.getString(map, "REMARK"), MapUtils.getString(map, "PRIMARY_TRANSACTION_ID"),
                MapUtils.getString(params, "serviceSerial") };
    }

    private String createInsertDepPrvncIntfLogSql() {
        StringBuffer sql = new StringBuffer();
        sql.append("insert into DEP_PRVNC_INTF_LOG (").append("ID, INTF_TYPE, TRANSACTION_ID,").append(
                "ACTION_CODE, BUS_CODE, SVC_CODE,").append("STATE, CREATE_DT, VERSION, ").append(
                "REMARK, PRIMARY_TRANSACTION_ID,SERVICE_SERIAL)");
        sql.append(" values (").append("?,?,?,").append("?,?,?,").append("?,sysdate,sysdate,").append("?,?,?)");
        return sql.toString();
    }

    private Object[] createInsertDepPrvncIntfLogInfoSqlParams(Map params) {
        Map map = (Map) params.get("LogInfo");
        return new Object[] { MapUtils.getString(params, "seq"), MapUtils.getString(map, "REQ_PARAM"),
                MapUtils.getString(map, "RES_PARAM") };
    }

    private String createInsertDepPrvncIntfLogInfoSql() {
        StringBuffer sql = new StringBuffer();
        sql.append("insert into DEP_PRVNC_INTF_LOG_INFO (").append("ID, REQ_PARAM, RES_PARAM)");
    ...
    private Object[] createInsertLmcWriteCardEventSqlParams(Map params) {
        ...
        return new Object[] {
                ...
                MapUtils.getString(map, "DATA_AREA"), MapUtils.getString(map, "UPDATE_TIME"),
                MapUtils.getString(map, "STAFF_IP"), MapUtils.getString(map, "REMARK"),
                MapUtils.getString(map, "STAFF_AREA"), MapUtils.getString(map, "SALE_NO"),
                MapUtils.getString(map, "EXT_CUST_ORDER_ID"), MapUtils.getString(params, "TRANSACTION_ID"),
                MapUtils.getString(params, "serviceSerial") };
    }
    ...
    private String createInsertLmcWriteCardEventSql() {
        ...
        StringBuffer sql = new StringBuffer();
        sql.append("insert into LMC_WRITE_CARD_EVENT (").append("EVENT_ID, EVENT_TYPE, ICCID,").append(
                "CARD_TYPE, CARD_NO, CARD_AREA,").append("MAKE_TIME, DLL_ID, STATE,").append(
                "STAFF_ID, CHANNEL_ID, DATA_AREA,").append("UPDATE_TIME, STAFF_IP, REMARK,").append(
                "STAFF_AREA, SALE_NO, EXT_CUST_ORDER_ID,").append("TRANSACTION_ID,SERVICE_SERIAL)");
        sql.append(" values (").append("?,?,?,").append("?,?,?,").append("sysdate,?,?,").append("?,?,?,").append(
                "?,?,?,").append("?,?,?,").append("?,?)");
        return sql.toString();
    }
}
