 ...
 * 物联网过户反档
 */
public class IOTTransferArchive extends Service{
    ...
    public DataMap exec(DataMap dataMap, String serviceSerial) throws Exception {
        ...
        try {
        	String[] params = { "acctOrderItemGrp","custOrderInfo","custOrderItemGrp","prodOrderItemGrp" };
    		dataMap = DataMapUtil.checkParam(dataMap, params);
    		if (StringUtils.isNotBlank(dataMap.getResultCode())) {
    			return dataMap;
    		}
    		
			String inXML = TcpCont.parseTemplate(dataMap.getInParam(), this.getClass().getSimpleName());
            ...
            inMap.put("in0", inXML);
	    	String url = DataRepository.getInstence().getSysParamValue(LteConstants.IOT_CSB_URL_KEY, SysConstant.CON_SYS_PARAM_GROUP_INTF_URL);
			...
			if (ResultConstant.R_POR_SUCCESS.getCode().equals(MapUtils.getString(resMap, "resultCode"))){
				String resXml = TcpCont.buildInParam(MapUtils.getMap(resMap, "result"), this.getClass().getSimpleName()+"Res");
